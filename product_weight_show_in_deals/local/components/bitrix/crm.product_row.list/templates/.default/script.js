...
if (typeof(BX.CrmProduct) === "undefined")
{
	...
	BX.CrmProduct.prototype =
	{
		...
		layout: function()
		{
			...
			/*this.getProductsPrice(productId);*/
            var href = window.location.href, matches, dealId;
            var isTaxAllowed = this._editor.isTaxAllowed();
            var isTaxIncluded = this.isTaxIncluded();
            ...
            console.log(dealId);
            if(dealId == 0) {
            	alert('Сохраните сделку, затем добавьте товар!');
            	return false;
			}
			if(dealId > 0) {
                var curapp = this;
                BX.ajax({
                    url: '/local/ajax/changeTypePrice.php',
                    data: {'id': productId, 'deal_id': dealId, 'action': 'getProductsPrice'},
                    method: 'POST',
                    dataType: 'json',
                    timeout: 30,
                    async: true,
                    ...
                    cache: false,
                    onsuccess: function (data) {
                        //console.log(data.PRICE_VALUE);
                        // PRICE NETTO
                        var priceNetto = curapp.getSetting('PRICE_NETTO', 0.0);
                        //var priceNetto = curapp.getSetting('PRICE_NETTO', 0.0);
                        curapp.setFieldValue('PRICE_NETTO', priceNetto.toFixed(2));

                        // PRICE BRUTTO
                        /*var priceBrutto = (data.PRICE_VALUE) ? data.PRICE_VALUE : curapp.getSetting('PRICE_BRUTTO', 0.0);
                        if(data.PRICE_VALUE) {
                            priceBrutto = curapp._round(BX.CrmProduct.calculateInclusivePrice(priceBrutto, curapp._settings['TAX_RATE']), 2);
                            curapp.setFieldValue('PRICE_BRUTTO', priceBrutto);
                        } else {
                            curapp.setFieldValue('PRICE_BRUTTO', priceBrutto.toFixed(2));
                        }*/

                        var priceBrutto = curapp.getSetting('PRICE_BRUTTO', 0.0);
                        curapp.setFieldValue('PRICE_BRUTTO', priceBrutto.toFixed(2));

                        // QUANTITY
                        var showQuantity = curapp.getSetting("QUANTITY", 0.0);
                        curapp.setFieldValue("QUANTITY", showQuantity);

                        // MEASURE
                        var defMeasureCode = "";
                        var defMeasureInfo = curapp._editor.getSetting("defaultMeasure", null);
                        if (defMeasureInfo) {
                            if (defMeasureInfo.hasOwnProperty("CODE"))
                                defMeasureCode = defMeasureInfo["CODE"];
                        }
                        curapp.setFieldValue("MEASURE_CODE", curapp.getSetting("MEASURE_CODE", defMeasureCode));

                        // DISCOUNT TYPE
                        var discountType = curapp.getDiscountTypeId();
                        curapp.setFieldValue("DISCOUNT_TYPE_ID", discountType);

                        // DISCOUNT RATE
                        var showDiscount = curapp.getSetting("DISCOUNT_RATE", 0.0);
                        curapp.setFieldValue("DISCOUNT_RATE", showDiscount);

                        // DISCOUNT SUM
                        var showDiscountSum = curapp.getSetting("DISCOUNT_SUM", 0.0);
                        curapp.setFieldValue("DISCOUNT_SUM", showDiscountSum.toFixed(2));

                        // DISCOUNT SUBTOTAL
                        var showDiscountSubtotal = showQuantity * showDiscountSum;
                        curapp.setFieldValue("DISCOUNT_SUBTOTAL", showDiscountSubtotal.toFixed(2));
                        ...
                        if (isTaxAllowed) {
                            ...
                            var defTaxRateInfo = curapp._editor.getSetting("defaultTax", null);
                            if (defTaxRateInfo) {
                        ...
                        curapp.setFieldValue("TAX_RATE", curapp.getSetting("TAX_RATE", defTaxRateValue));

                        // TAX INCLUDED
                        curapp.setFieldValue("TAX_INCLUDED", curapp.isTaxIncluded());

                        // TAX SUM
                        curapp.setFieldValue("TAX_SUM", curapp._round(curapp.getSetting("TAX_SUM", 0.0), 2));
                        // SUM
                        curapp.setFieldValue("SUM", curapp._calculateSumTotal());

                        curapp._handleProductNameChange(productName);

                        curapp._bindEventsHandlers();
                        curapp.initializeDragDropAbilities();

                        curapp._hasLayout = true;
                        if (data.PRICE_VALUE) {
                            $("#" + row.id + "_PRICE").val(data.PRICE_VALUE);
                            $("#" + row.id + "_PRICE").simulate("keyup", {keyCode: 106});
                            console.log(row.id);
                        }
