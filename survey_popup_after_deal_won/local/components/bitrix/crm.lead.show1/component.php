...
if (!CCrmLead::CheckReadPermission($arResult['ELEMENT_ID'], $userPermissions))
{
...
$arFields['LEAD_CONVERT_SCRIPT'] = "BX.CrmLeadConverter.getCurrent().convert({$arFields['~ID']}, BX.CrmLeadConversionScheme.createConfig('".\Bitrix\Crm\Conversion\LeadConversionScheme::DEAL_CONTACT_COMPANY_NAME."'), '".CUtil::JSEscape($APPLICATION->GetCurPage())."');";
...
if ($arResult['ELEMENT']['STATUS_ID'] == 'CONVERTED')
{
	...
	if (!$userPermissions->HavePerm('DEAL', BX_CRM_PERM_NONE, 'READ'))
	{
		ob_start();
