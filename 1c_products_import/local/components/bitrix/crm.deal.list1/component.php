...
if (!empty($sExportType) && $userPermissions->HavePerm('DEAL', BX_CRM_PERM_NONE, 'EXPORT'))
{
	ShowError(GetMessage('CRM_PERMISSION_DENIED'));
	return;
}
...
$arResult['PERMS']['ADD']    = !$userPermissions->HavePerm('DEAL', BX_CRM_PERM_NONE, 'ADD');
$arResult['PERMS']['WRITE']  = !$userPermissions->HavePerm('DEAL', BX_CRM_PERM_NONE, 'WRITE');
$arResult['PERMS']['DELETE'] = !$userPermissions->HavePerm('DEAL', BX_CRM_PERM_NONE, 'DELETE');
...
{
	if ($userPermissions->GetPermType('DEAL', 'WRITE', array('STAGE_ID'.$sStatusId)) > BX_CRM_PERM_NONE)
...
if($actionData['ACTIVE'])
{
	if ($actionData['METHOD'] == 'POST')
	{
		if($actionData['NAME'] == 'delete')
		{
			if ((isset($actionData['ID']) && is_array($actionData['ID'])) || $actionData['ALL_ROWS'])
			{
				...
				{
					$ID = $arDeal['ID'];
					$arEntityAttr = $userPermissions->GetEntityAttr('DEAL', array($ID));
					if (!$userPermissions->CheckEnityAccess('DEAL', 'DELETE', $arEntityAttr[$ID]))
					{
						continue ;
					}
		...
		elseif($actionData['NAME'] == 'edit')
		{
			if(isset($actionData['FIELDS']) && is_array($actionData['FIELDS']))
			{
				...
				{
					$arEntityAttr = $userPermissions->GetEntityAttr('DEAL', array($ID));
					if (!$userPermissions->CheckEnityAccess('DEAL', 'WRITE', $arEntityAttr[$ID]))
					{
						continue ;
					}
	...
	else//if ($actionData['METHOD'] == 'GET')
	{
		if ($actionData['NAME'] == 'delete' && isset($actionData['ID']))
		{
			...
			$arEntityAttr = $userPermissions->GetEntityAttr('DEAL', array($ID));
			if(CCrmAuthorizationHelper::CheckDeletePermission(CCrmOwnerType::DealName, $ID, $userPermissions, $arEntityAttr))
			{
				$DB->StartTransaction();
...
$arResult['PAGINATION']['URL'] = $APPLICATION->GetCurPageParam('', array('apply_filter', 'clear_filter', 'save'));
...
if (isset($arResult['DEAL_ID']) && !empty($arResult['DEAL_ID']))
{
	...
	{
		$arResult['DEAL'][$iDealId]['EDIT'] = $userPermissions->CheckEnityAccess('DEAL', 'WRITE', $arDealAttr[$iDealId]);
		$arResult['DEAL'][$iDealId]['DELETE'] = $userPermissions->CheckEnityAccess('DEAL', 'DELETE', $arDealAttr[$iDealId]);
		...
		foreach ($arBPData as $arBP)
		{
			if (!CBPDocument::CanUserOperateDocument(
				...
				array(
					...
					'DocumentStates' => $arDocumentStates,
					'WorkflowTemplateId' => $arBP['ID'],
					'CreatedBy' => $arResult['DEAL'][$iDealId]['~ASSIGNED_BY_ID'],
					'UserIsAdmin' => $isAdmin,
					'CRMEntityAttr' => $arDealAttr
				)
			))
			{
				continue;
