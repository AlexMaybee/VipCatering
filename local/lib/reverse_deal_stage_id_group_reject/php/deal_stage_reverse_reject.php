...
class Group37ReverseStageIdMoveReject{
    ...
    public function checkUserGroupIf37(&$arFields){
                        ...
                        if(count($dealResult) > 0){
                            ...
                            if($dealResult[0]['CATEGORY_ID'] === '0'){
                                ...
                                if($arFields['STAGE_ID'] != $dealResult[0]['STAGE_ID']){
                                    ...
                                    //берем стадию WON
                                    if($dealResult[0]['STAGE_ID'] == 'WON' && (in_array($arFields['STAGE_ID'],['NEW','PROPOSAL',1,5,4,'LOSE']))) $error = 'Движение в направлении "Кейтеринг" по воронке в обратно направлении запрещено!';

                                    //берем стадию LOSE
                                    if($dealResult[0]['STAGE_ID'] == 'LOSE' && (in_array($arFields['STAGE_ID'],['NEW','PROPOSAL',1,5,4,'WON']))) $error = 'Движение в направлении "Кейтеринг" по воронке в обратно направлении запрещено!';
        ...
        self::logData($path,[$curUserId,$usersIdsfrom37group,$arFields,$dealResult]);
    }
