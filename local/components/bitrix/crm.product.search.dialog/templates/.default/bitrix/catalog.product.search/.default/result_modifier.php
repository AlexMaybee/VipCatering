...
{
    $deal_id_custom = current(explode('?', $GLOBALS['_SERVER']['HTTP_REFERER']));
    $deal_id = explode("/", $deal_id_custom);
    $link = array_diff($deal_id, array(''));
    $id = end($link);
	...
	if (is_array($arItems['PRICES']) && isset($arItems['PRICES'][$priceTypeId]))
	{
		...
		if($id > 0 && strstr($deal_id_custom, '/deal/')) {
            $arResult['PRODUCTS'][$productID]['PRICE'.$priceTypeId] = getPriceProduct($id, $productID). ' грн.';
        }
...
function getPriceProduct($id, $prod_id)
{
    $category_id = CCrmDeal::GetByID($id);
    ...
    $arFilter = Array("IBLOCK_ID"=> 59, "ID" => $prod_id);
