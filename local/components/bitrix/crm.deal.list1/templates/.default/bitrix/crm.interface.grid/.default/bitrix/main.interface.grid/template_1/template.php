...
//Can't call Reset() - there are another controls on page can use jsDD
jsDD.Enable();
...
if(is_array($ajaxLoaderParams))
{
	...
	?><script type="text/javascript">
		BX.GridAjaxLoader.remove("<?=$arParams["AJAX_ID"]?>");
		bxGrid_<?=$arParams["GRID_ID"]?>.ajaxLoader = BX.GridAjaxLoader.create("<?=$arParams["AJAX_ID"]?>", <?=CUtil::PhpToJSObject($ajaxLoaderParams)?>);
	</script>
	<?
}
